...
class _HomePageState extends State<HomePage> {
                ...
                GetX<WeatherController>(builder: (controller) {
                  return controller.isLoading.value
                      ...
                      : DailyWeatherCard(
                          ...
                          image: controller.weatherData.value.weather![0].icon,
                          temp: controller.weatherData.value.main!.temp,
                          description: controller
                              .weatherData.value.weather![0].description,
                          minTemp: controller.weatherData.value.main!.tempMin,
                          maxTemp: controller.weatherData.value.main!.tempMax,
                          feelsLike:
                              controller.weatherData.value.main!.feelsLike,
                          timeZone: controller.weatherData.value.timezone,
                        );
                }),
                const CustomDivider(),
                GetX<OneCallDailyController>(
                  builder: (controller) {
                    return controller.isLoading.value
                        ...
                        : WeeklyWeatherCard(
                            daily: controller.oneCallDailyData.value.daily,
                          );
                  },
                ),
                const CustomDivider(),
                GetX<OneCallHistoryController>(builder: (controller) {
                  return controller.isLoading.value
                      ? const SizedBox(
                          height: 300,
                          child: Center(
                      ...
                      : HistoryWeatherCard(
                          image: controller.oneCallHistoryData.value.current!
                              .weather![0].icon,
                          temp:
                              controller.oneCallHistoryData.value.current!.temp,
                          description: controller.oneCallHistoryData.value
                              .current!.weather![0].description,
                          humidity: controller
                              .oneCallHistoryData.value.current!.humidity,
                          feelsLike: controller
                              .oneCallHistoryData.value.current!.feelsLike,
                          date: controller.oneCallHistoryData.value.current!.dt,
                        );
                }),
              ],
            ),
  ...
  void activateApi() {
    _geocodingController.getGeocodingApi('London');
